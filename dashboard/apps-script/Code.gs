...
function buildWidgetLooksmax_() {
  ...
  var out = { generated_at: stamp, key: 'looksmax', name: 'looksmax', track_from: LM_TRACK_FROM,
    ...
    funnel_steps: [['land','Визит лендинга','teal'],['prod','Открыл продукт','teal'],
      ['quiz_start','Начал квиз','teal'],['gen','Нажал генерацию','teal'],
      ['pay_view','Увидел пейволл','amber'],['buy','Купил (однораз+подписка)','violet']],
    funnel_hint: 'Воронка-СПИНА до покупки: визит → продукт → квиз → генерация → пейволл → покупка (только надёжные монотонные шаги). ' +
      'Между генерацией и пейволлом есть рега-стена, скан, тизер, клики по локам — у них баги трекинга (рега-стена и scan недосчитывают, ' +
      'тизер фаерится на возврате без нового скана), поэтому в основную воронку их не ставим: квиз-детали смотри в панели «Квиз», ' +
      'клики по локам — в «Локах тизера», регистрацию — в KPI «рега после стены» (floor). Пейволл (get feature view) — надёжный гейт перед оплатой.',
...
function buildWidgetRMF_() {
  ...
  var out = { generated_at: stamp, key: 'ai-rate-my-face', name: 'rate-my-face', track_from: '2026-06-01',
    ...
    funnel_steps: [['land','Визит лендинга','teal'],['prod','Открыл продукт','teal'],
      ...
      ['buy','Купил (однораз+подписка)','violet']],
    funnel_hint: 'Воронка ПОКУПКИ: визит → продукт → пейволл → last-chance → покупка (только надёжные монотонные шаги). ' +
      'Пейволл (get feature view) — надёжный гейт перед оплатой. Регистрация (рега-стена sign up view) в воронку НЕ входит — ' +
...
function buildWidgetAP_() {
  ...
  var out = { generated_at: stamp, key: 'ai-add-person-to-photo', name: 'add-person-to-photo', track_from: '2026-06-01',
    ...
    funnel_steps: [['land','Визит лендинга','teal'],['prod','Открыл продукт','teal'],
      ['pay_view','Увидел пейволл','amber'],['buy','Купил (однораз+подписка)','violet']],
    funnel_hint: 'Воронка ПОКУПКИ: визит → продукт → пейволл → покупка. Пейволл (get feature view) — гейт перед оплатой, ' +
      'его проходят почти все дошедшие до покупки (97% из них — НОВЫЕ юзеры, зарегавшиеся в этой же сессии, а не возвращенцы). ' +
      'Рега-стену (sign up view) в воронку НЕ ставим: её событие сильно недосчитывает (здесь зафаерилось лишь у ~63% реально ' +
      'зарегавшихся — баг трекинга), поэтому она меньше пейволла — это НЕ значит, что юзеры её пропускают. Регистрацию смотри в ' +
