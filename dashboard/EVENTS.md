...
## Урок про воронки: рега-стена и пейволл — РАЗНЫЕ гейты, не выстраивать в линию (23.07)

- **`sign up view` (рега-стена) и `get feature view` (пейволл) — параллельные гейты для разных состояний логина**, НЕ последовательные шаги. Разлогиненный видит рега-стену; залогиненный (returning) видит пейволл, минуя регистрацию. add-person за 7д: рега-стену 303, пейволл 505, оба 212, только пейволл (залогинены) 293, только стена 91.
- **Симптом ошибки**: если ставить wall→reg→pay последовательно, «% пред» скачет >100% (пейволл больше реги), воронка «хипхопит». Лиза поймала на add-person.
- **РЕШЕНИЕ (по требованию Лизы — НЕ склеивать рега+пейволл)**: воронка ПОКУПКИ идёт через ПЕЙВОЛЛ как универсальный гейт перед оплатой (его проходят и залогиненные, и разлогиненные): land → prod → пейволл → покупка (монотонно 981>673>500>66). Регистрация — ОТДЕЛЬНАЯ ветка для разлогиненных, живёт в KPI «рега после стены», НЕ в воронке покупки. Опциональные шаги (виджет-онбординг лендинга) — тоже вне воронки.
- **Не делать**: не склеивать рега+пейволл в один «гейт» (Лиза отвергла), не ставить их последовательно (хипхоп). Пейволл = гейт покупки; регистрация = отдельно.
- Соотношение wall vs pay зависит от виджета: RMF (SEO, новые) — стена>>пейволл; add-person (returning) — пейволл>стена. Воронку строить через пейволл везде для консистентности.
