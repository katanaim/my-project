...
- **`sign up view` (рега-стена) и `get feature view` (пейволл) — параллельные гейты для разных состояний логина**, НЕ последовательные шаги. Разлогиненный видит рега-стену; залогиненный (returning) видит пейволл, минуя регистрацию. На add-person за 7д: рега-стену видели 303, пейволл 505, ОБА — 212, только пейволл (залогинены) — 293, только рега-стену — 91. Union = 596.
- **Симптом ошибки**: если ставить их в воронку как wall→reg→pay последовательно, «% пред» скачет >100% (пейволл больше реги → 260%), воронка «хипхопит» и выглядит сломанной. Лиза поймала это на add-person.
- **Правильно**: объединять в один шаг «гейт = рега-стена ИЛИ пейволл» (distinct union), тогда воронка монотонна: land → prod → гейт → (рега) → покупка. Регистрацию показывать отдельным KPI, не как гейт покупки.
- **Соотношение wall vs pay зависит от виджета**: RMF (огромный SEO, новые юзеры) — стена >> пейволл, воронка монотонна сама собой. add-person (много returning/залогиненных) — пейволл > стена, нужен union. Проверять на каждом виджете.
- **Опциональные шаги (виджет-онбординг на лендинге) не ставить в основную вертикальную воронку** — им пользуется меньшинство, даёт провал→скачок. Выносить отдельно/в KPI.
