...
| `purchase_apple_*` / `purchase_google_*` | — | — | плоские дубли wallet-флоу (`purchase_google_*` — только с 24.07.2026) |
| `purchase_stripe_embedded_*` | — | — | embedded-чекаут картой, плоское (только с 24.07.2026) |

**Wallet-флоу (Apple/Google Pay):** confirmCardPayment на клиенте → ecommerce-события + канон `purchase/<apple|google>/<план>` в overchat-схеме (`eventAction='google'` — новая фича с 24.07.2026). **Карта (redirect):** только dataLayer-события, канona overchat почти нет — `purchase/None/<план>` это карт-флоу, где eventAction не передан. **Embedded-карта** (с 24.07): `purchase_stripe_embedded_*` + канон `eventAction='stripe_embedded'`.

**🔴 С 24.07.2026 `subscription_started` ДВАЖДЫФАЕРИТСЯ** (новый чекаут Google Pay + embedded): ~1.5–2× событий на юзера. Подписки — ТОЛЬКО `COUNT(DISTINCT user)`, никогда `COUNT(*)`. Дашборд не затронут (считает distinct).
...
## Виджет «ai change text in photo» — лендинг-онли, разбор 24.07

- **Где живёт**: лендинг `edit-text-in-image`, семейство событий `change_text_in_photo_*`, с 31.05.2026. Продукт-страницы /web/ НЕТ — только лендинг-инструмент (без монетизации).
- **Инструментация подробная и настроена**: click_upload, upload/upload_success/upload_fail, ocr_start/ocr_done/ocr_fail, click_sample, select_text, confirm_text_edit, click_generate, generate_success/fail, download.
- **Сколько пользуется** (вся история с 31.05, уники-юзеро-дни): визиты лендинга 1356 → загрузили фото 627 (46%) → OCR распознал текст 602 (44%). Картинок на юзера: медиана 1, среднее 1.29 (473 юзера×1, 125×2, 22×3), всего 800 картинок.
- **🔴 СИСТЕМНЫЙ ОБРЫВ ПОСЛЕ OCR**: из 602 с распознанным текстом только 26 (4%) выбрали текст (select_text), 18 нажали генерацию, 17 успешно сгенерили, 6 скачали. До результата (download) доходит 6 из 1356 (0.4%). OCR здоров (fail ~5%), обрыв именно на шаге редактирования.
- **Не мобильный баг — на обоих устройствах**: моб OCR 429 → выбрал текст 15 (3.5%); деск OCR 174 → 3 (1.7%). Значит либо сломан/непонятен UX редактирования после OCR, либо события select_text/generate не фаерятся. ВОПРОС ДЕВАМ: почему 96% отваливаются после OCR — проверить флоу «выбрать/отредактировать текст» и фаеринг select_text/confirm_text_edit/click_generate.

## 🟢 Google Pay + новый чекаут — запуск 24.07.2026 (пятница); разбор всплеска подписок (27.07)

**Что выкатили:** в пятницу 24.07 добавили оплату **Google Pay** (своя фича, события «как у Apple Pay») + **embedded-чекаут картой** (`purchase_stripe_embedded_*`). До 24.07 в GA этих методов не было ВООБЩЕ (0 событий), всё не-Apple шло картой-redirect.

**Где живут события Google Pay** (дедуп как у Apple — тройное логирование):
- канон `event_name='overchat'`, `eventCategory='purchase'`, `eventAction='google'`, `eventLabel=<план>` — берём;
- плоское `purchase_google_<план>` (`purchase_google_package-onetime`, `purchase_google_pro_weekly`, …) — дубль;
- `event_name='user_id_pushed'` — дубль, дропать.

**🔴 Побочка: `subscription_started` дваждыфаерится с 24.07** — новый чекаут шлёт событие ~1.5–2× на юзера (24–26.07: 178 событий / 112 юзеров; 39 юзеров задвоили, у каждого один и тот же план дважды = ОДНА подписка). **Подписки считать ТОЛЬКО `COUNT(DISTINCT user_pseudo_id)`.** Дашборд (Code.gs) уже так и делает → не затронут. Onetime-контаминации в подписки нет (проверено: 6 Google-Pay-одноразок, попавших в subscription_started, — легальные апгрейдеры с реальным планом).

**Разбивка методов оплаты (кол-во ЧЕЛОВЕК, эксклюзивно 1 юзер=1 метод):**
| Период | Apple Pay | Карта-redirect | Stripe embedded | Google Pay | всего/день |
|---|---:|---:|---:|---:|---:|
| до 24.07 (08–23) | ~92/день (70%) | ~39/день (30%) | 0 | ~0 | ~131 |
| скачок 24–26 | ~145/день (68%) | ~15/день (7%) | ~29/день (14%) | ~21/день (10%) | ~211 |

Новый чекаут частично **каннибализировал** карту-redirect (39→15/день), частично добавил объёма. Google Pay пока в основном для одноразок $2.99 (54 из 68 gpay-юзеров — onetime, лишь 16 — подписки).

**Всплеск подписок 24–26 = НЕ приток трафика, а рост конверсии в ПЛАТНОМ сегменте.** Трафик плоский (~15k визитов/день), пейволл-воронка плоская, но visit→sub вырос ×1.6 (0.15%→0.24%), и целиком в cpc: платный визит→подписка 0.63%→0.88% (×1.4) + платный трафик ×1.8 по объёму; органика не двинулась (0.075→0.091, шум). 87% прироста подписок — платный канал. План — 80% weekly $5.99. Апгрейд одноразка→подписка восстановился (провал 13–23.07 ~3.8% → 24–26 ~7.8%). Метод расчёта конверсии по каналам: `traffic_source.medium` (cpc=платный, organic=органика) на page_view (знаменатель) и subscription_started (числитель), distinct users.
